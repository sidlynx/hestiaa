...
class UserTokenMiddleware {
  ...
  static getHandler () {
    return (req, res, next) => {
      ...
      if (req.headers.authorization) {
        let authorizationHeadersStrings = req.headers.authorization.split(' ')
        if (authorizationHeadersStrings[0] === 'Bearer') {
          queryToken = authorizationHeadersStrings[1]
        } else {
      ...
      try {
        req.tokenPayload = jwt.verify(queryToken, process.env.APP_KEY)
        next()
      } catch (err) {
        next(err)
      }
  ...
  static isRoutePublic (req) {
    return this.publicRoutes().find(r => {
      return r.method === req.method && new UrlPattern(r.urlPattern).match(url.parse(req.url).pathname)
    })
  }
