const ModelClass = require('mongorito').ModelClass

/**
 * @param requestQuery {@link express~Request#query}
 * @see mongorito~Model.find
 */
export async function findPaginated (model, requestQuery, query) {
  ...
  return new PaginatedResult(result, Math.ceil(offset / count), totalItems / count)
}
