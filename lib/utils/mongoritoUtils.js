...
class MongoritoUtils {
  /**
   * Separated in order to be mocked
   * @private
   */
  static _now () {
    return new Date()
  ...
  static _fillFields (model, fields) {
    const timestamp = MongoritoUtils._now()
 ...
 */
MongoritoUtils.rootTraceabilityPlugin = MongoritoUtils._generateTraceabilityPlugin([ActionTypes.SAVE])
/**
 * Traceability plugin for *embedded* document.
 * Distinct implementation, because `SAVE` action cannot be intercepted, because `.save()` is called on *root* document.
 * @see MongoritoUtils#_generateTraceabilityPlugin
 */
MongoritoUtils.embeddedTraceabilityPlugin = MongoritoUtils._generateTraceabilityPlugin([ActionTypes.SET, ActionTypes.UNSET])
